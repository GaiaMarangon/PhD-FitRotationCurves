...
QVec = [0.75,0.8,0.85,0.9,1.0,1.1,1.25]; 
nVec = [2,4,5,6,7,8,9,  10,12];    
...
%--- PARAMETER RANGES
xSup = 100;         %upper limit for parameter x

%--- EXPERIMENTAL REFERENCE FEATURES PARAMETERS
...
%--- NUMERICAL CONSTANTS (SCALING PARAMETERS)
...
%load available parameter combinations (if any)
...
for iQ=1:length(QVec)
    ...
    Rout = r(end);
    Vout = vDM_exp(end);
    % TEMPORARY: visual check of reference points of vDM_exp
    figure()
    hold on
    plot(r,vDM_exp,'--ok','DisplayName',"$V_{DM}^{Exp}$")
    plot(Rin,Vin,'x','DisplayName',"$(R_{in},V_{in})$")
    plot(r(idxStart:idxStop),slope*r(idxStart:idxStop)+interc,"-k","DisplayName","V = "+string(round(slope,2))+"R+"+ string(round(interc,2)));
    
    % n parameter
    for in=1:length(nVec)
        n = nVec(in);
        % compute check n parameter
        check1 = (a_rOut*n^2 +b_rOut*n +c_rOut)/(m_rInn*n+q_rInn);
        check2 = beta_vOut/beta_vInn *(a_rOut*n^2 +b_rOut*n + c_rOut)^alpha_vOut/(m_rInn*n + q_rInn)^alpha_vInn;
        if (check1> Rout/Rin) && (check2 > Vout/Vin)
            ...
            % estimate m,x parameters
            mEst = v_scale/Vin *beta_vInn*(m_rInn*n+q_rInn)^(alpha_vInn+1) *r_scale/Rin;
            xEst = Vin/v_scale  /beta_vInn/(m_rInn*n+q_rInn)^alpha_vInn /mEst;
            mVec = [floor(mEst*100)/100-0.01,floor(mEst*100)/100,ceil(mEst*100)/100,ceil(mEst*100)/100+0.01]; 
            xVec = [floor(xEst)-1,floor(xEst),ceil(xEst),ceil(xEst)+1];       
                ...
                if x<xSup
                    ...
                    for im=1:length(mVec)
                        ...
                        if any(isComputed) 
                            ...
                            %--- END SIMULATE COMBINATION ----------------
        
                            % add accepted combination with chi2 to table
                            paramTable = [paramTable; [paramCombo, chi2]];
                            ...
                            fprintf("Computed combo: Q = "+string(Q) +", n = "+string(n) +", x = "+string(x) +", m = "+string(m)+"\n")
                            fprintf("Computed %d-th combo out of %d\n\n",iProgress,length(QVec)*length(nVec)*length(xVec)*length(mVec));
                            % update list of computed combinations
                            save(pathfile+paramTableFile,'paramTable')
                        end
                    end
                else
                    % check on x not passed
                    iProgress = iProgress+length(mVec);
                    fprintf("Computed combo: Q = "+string(Q) +", n = "+string(n) +", x = "+string(x) +", m = --- \n")
                    fprintf("Exceeding x, combinations not computed\n")
                    fprintf("Computed %d-th combo out of %d\n\n",iProgress,length(QVec)*length(nVec)*length(xVec)*length(mVec));
                end
            end
        else
            % checks on outer point not passed
            iProgress = iProgress+length(xVec)*length(mVec);
            fprintf("Computed combo: Q = "+string(Q) +", n = "+string(n) +", x = ---, m = --- \n")
            fprintf("Checks on outer point not passed, combinations not computed\n")
            fprintf("Computed %d-th combo out of %d\n\n",iProgress,length(QVec)*length(nVec)*length(xVec)*length(mVec));
        end
    end
end
